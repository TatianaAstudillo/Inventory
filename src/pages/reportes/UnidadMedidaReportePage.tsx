...
const UnidadMedidaReportePage: React.FC = () => {
  ...
  const handleFilter = () => {
    ...
    const filtered = unidadesMedida.filter(unidadMedida => {
      if (!unidadMedida.fechaCreacion) return false;
      
      const creationDate = new Date(unidadMedida.fechaCreacion);
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredUnidadesMedida.map((unidadMedida) => (
                    <tr key={unidadMedida.idUnidadMedida}>
                      <td className="px-6 py-4 whitespace-nowrap">{unidadMedida.idUnidadMedida}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{unidadMedida.unidadMedida}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${unidadMedida.activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                          {unidadMedida.activo ? 'Activo' : 'Inactivo'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {unidadMedida.fechaCreacion ? new Date(unidadMedida.fechaCreacion).toLocaleDateString('es-ES') : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default UnidadMedidaReportePage;